            ...
            <tr class="hover:bg-gray-100">
                ...
                <td class="p-4 whitespace-nowrap text-sm">
                    <span class="px-2 py-1 text-xs rounded-full 
                        {{ $u->level == 'admin' ? 'bg-cyan-100 text-cyan-800' : 'bg-gray-100 text-gray-800' }}">
                        {{ ucfirst($u->level) }}
                    </span>
                </td>
                <td class="p-4 whitespace-nowrap space-x-2 text-center">
                    ...
                    <form action="{{ route('user.destroy', $u->id) }}" method="POST" class="inline delete-form">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
    ...
    <div class="bg-white rounded-lg shadow w-full max-w-2xl relative">
        ...
        <form action="{{ route('user.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-6 gap-6">
                ...
                <div class="col-span-6 sm:col-span-3">
                    <label for="level" class="text-sm font-medium text-gray-900 block mb-2">Level</label>
...
<div id="edit-user-modal-{{ $aa->id }}"
    ...
    <div class="bg-white rounded-lg shadow w-full max-w-2xl relative">
        ...
        <div class="flex items-start justify-between p-5 border-b rounded-t">
            ...
            <button type="button"
                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                data-modal-toggle="edit-user-modal-{{ $aa->id }}">
        ...
        <!-- Form Edit -->
        <form action="{{ route('user.update', $aa->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6 sm:col-span-3">
                    <label for="name-{{ $aa->id }}" class="text-sm font-medium text-gray-900 block mb-2">Nama</label>
                    <input type="text" name="name" id="name-{{ $aa->id }}" value="{{ $aa->name }}"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                        required>
                </div>
                ...
                <div class="col-span-6 sm:col-span-3">
                    ...
                    <input type="email" id="email-{{ $aa->id }}" value="{{ $aa->email }}" readonly
                        class="shadow-sm bg-gray-100 border border-gray-300 text-gray-500 sm:text-sm rounded-lg block w-full p-2.5">
                </div>

                <div class="col-span-6 sm:col-span-3">
                    <label for="level-{{ $aa->id }}" class="text-sm font-medium text-gray-900 block mb-2">Level</label>
                    <select name="level" id="level-{{ $aa->id }}"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5">
                        <option value="admin" {{ $aa->level == 'admin' ? 'selected' : '' }}>Admin</option>
                        <option value="petugas" {{ $aa->level == 'petugas' ? 'selected' : '' }}>Petugas</option>
                    </select>
                </div>
